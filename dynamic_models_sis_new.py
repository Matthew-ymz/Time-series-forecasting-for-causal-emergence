...
class Simple_Spring_Model():
    ...
    def multi_steps_sir(self, s, steps, sigma,lam=1,miu=0.5,rou=-1): 
        #One sample point runs multiple time steps.
        s_hist = s
        sn_hist = self.perturb(s, 0.001,rou)
        for t in range(steps):
            s_next = self.SIR_step(s[:,0],s[:,1],lam,miu)
    ...
    def SIR_step(self,s,i,lam,miu,dt=0.01):
        s=s-dt* lam * s * i
    ...
    def generate_multistep_sir(self,size_list,steps,lam=1,miu=0.5,sigma=1,rou=-1):
