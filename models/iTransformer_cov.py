...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.encoder = Encoder(
            ...
            output_attention=configs.output_attention and (not configs.is_training)
        )
        # Decoder
        ...
        self.fc_L = nn.Linear(configs.d_model, configs.pred_len, bias=True) 
    ...
    def forecast(self, x_enc):
        ...
        return dec_out, attns, L

    def imputation(self, x_enc, x_dec, mask):
        # Normalization from Non-stationary Transformer
        means = x_enc.mean(1, keepdim=True).detach()
        x_enc = x_enc - means
        stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
        ...
        return dec_out, attns[0] # taking 1st layer only
    ...
    def forward(self, x_enc, x_dec, mask=None, EI_bool=False):
        ...
        if self.task_name == 'imputation':
            ...
            result = dec_out  # [B, L, D]
        if self.task_name == 'anomaly_detection':
            dec_out, attn = self.anomaly_detection(x_enc)
            result = dec_out  # [B, L, D]
        if self.task_name == 'classification':
